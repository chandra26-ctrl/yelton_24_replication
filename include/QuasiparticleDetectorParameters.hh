////////////////////////////////////////////////////////
//
// QuasiparticleDetectorParameters.hh
//
// linehan3@fnal.gov
...
#ifndef QuasiparticleDetectorParameters_h
#define QuasiparticleDetectorParameters_h 1
...
//----------------------------------------------------------------
// Niobium info
constexpr double dp_polycryElScatMFP_Nb = 10.0 * CLHEP::nm; // Niobium ground-plane parameters
constexpr double dp_scDelta0_Nb = 0.001538 * CLHEP::eV; // Yelton Table I: 1538 micro-eV
constexpr double dp_scTeff_Nb = 0.8 * CLHEP::kelvin; // G4CMP validation defaults
constexpr double dp_scDn_Nb =
    1.4 * CLHEP::um * CLHEP::um / CLHEP::ns; // G4CMP validation defaults
constexpr double dp_scTauQPTrap_Nb = 1.0 * CLHEP::ms; // G4CMP validation defaults


  //----------------------------------------------------------------
  // Aluminum Josephson-junction electrode parameters used by Yelton et al.
  // Their effective electrode represents the combined 40 nm and 80 nm films.
  constexpr double dp_polycryElScatMFP_Al = 10 * CLHEP::nm;
  constexpr double dp_scDelta0_Al = 0.000191 * CLHEP::eV;
  constexpr double dp_scTeff_Al = 0.2 * CLHEP::kelvin;
  constexpr double dp_scDn_Al = 6 * CLHEP::um*CLHEP::um / CLHEP::ns;
  // s = 5e-2 /us in the paper, corresponding to a 20 us trapping time.
  constexpr double dp_scTauQPTrap_Al = 20.0 * CLHEP::us;
  ...
  constexpr double dp_siliconChipDimZ = 0.525 * CLHEP::mm;
 

  //----------------------------------------------------------------
  // The paper replaces explicit package transport with 2.5% phonon loss on
  // the four vertical silicon boundaries, so the copper housing stays off.
  ...
  // Simplified detector geometry: replace each of the six former resonator
  // assemblies with a 10 um square, 120 nm thick Al junction-electrode patch.
  // The patch centers retain the original three-by-two resonator positions.
  constexpr bool dp_usePaperAlPatchGrid = true;
  constexpr double dp_alPatchDimX = 10 * CLHEP::um;
  constexpr double dp_alPatchDimY = 10 * CLHEP::um;
  constexpr double dp_alPatchDimZ = 120 * CLHEP::nm;
  constexpr int dp_alPatchCountX = 3;
  constexpr int dp_alPatchCountY = 2;
